...
public class SetPointsCommand implements SimpleCommand{
    ...
    public SetPointsCommand(ProxyServer server, RangAPI rangapi) {
        this.server = server;
    ...
    public void execute(Invocation invocation) {
        ...
        try{
            rangapi.setPoints(targetPlayerUUID, amount);
            source.sendMessage(Component.text(amount + " points were written to player " + targetPlayer + "."));
            return;
        } catch (Exception e) {
            source.sendMessage(Component.text("Internal error: Points could not be set. Error in calling RangAPI"));
            return;
        }

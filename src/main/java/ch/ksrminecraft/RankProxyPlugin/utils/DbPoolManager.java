...
public class DbPoolManager {
    ...
    public DbPoolManager(String jdbcUrl, String user, String pass, LogHelper log) {
        ...
        log.info("DbPoolManager: HikariCP initialisiert ({})", jdbcUrl);

        // Kurztest
        try (Connection c = this.dataSource.getConnection()) {
            if (!c.isValid(2)) {
                log.warn("DbPoolManager: Testverbindung ist nicht gültig");
            } else {
                log.debug("DbPoolManager: Testverbindung erfolgreich validiert");
            }
        } catch (Exception e) {
            log.error("DbPoolManager: Konnte Testverbindung nicht öffnen: {}", e.getMessage());
            log.debug("DbPoolManager Exception beim Öffnen der Testverbindung", e);
        }
    }
    ...
    public void close() {
        if (dataSource != null && !dataSource.isClosed()) {
            log.info("DbPoolManager: Schließe HikariCP-Pool");
            dataSource.close();
        }
    }
}